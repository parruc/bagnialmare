...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        logger.warning("Restoring Municipalities and Districts")
        ...
        for bagno in bagni:
            ...
            try:
                ...
                b = Bagno.objects.filter(point__distance_lte=(point, 2))
                if len(b) == 1:
                    b = b[0]
                else:
            ...
            if not d:
                logger.warning("creating district %s" % politicals[2])
            ...
            if not m:
                logger.warning("creating municipality %s" % politicals[1])
            ...
            if not n:
                logger.warning("creating neighbourhood %s" % politicals[0])
